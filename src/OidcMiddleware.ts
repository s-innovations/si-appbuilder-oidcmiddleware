...
function createAuthorization(ctx: OIDCAppContext, user: Oidc.User, err?: string) {
    if (user) {
        ...
        console.log(user);
      //  ctx.appInsights.setAuthenticatedUserContext(user.profile.sub);
    } else {
...
export async function OidcMiddleware<T extends OIDCAppContext>(ctx:T , next: AppFunc<T>) {
    ...
    var mgr = new oidc.UserManager(config) as Oidc.UserManager;
    Object.defineProperty(ctx, "userManager", { value: mgr, enumerable: false });
    ...
    if (window.location.hash.indexOf("#id_token") === 0) {
        ...
        createAuthorization(ctx, await mgr.getUser())
        ...
        if (ctx.authorization.isSignedIn) {
            ...
            if (ctx.authorization.user.expired) {
                ...
                await mgr.signinSilent().then((user) => createAuthorization(ctx, user), (err) => createAuthorization(ctx, null, err));
